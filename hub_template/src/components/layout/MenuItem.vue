<template>
	<div class="layout-menu">
		...
		<div class="main-nav-bg" :class="{ 'el-aside-expand': isCollapse }">
					...
					<template v-for="menu in menus" :key="menu.path">
						<template v-if="!menu.meta?.exceptCreateMenu">
							<el-menu-item v-if="!menu.children.length" :index="menu.path" :key="menu.path">
								...
								<i v-if="menu?.iconfont" class="iconfont f-s-18" :class="menu?.iconfont"></i>
							...
							<el-sub-menu v-else :index="menu.path">
								...
								<template v-for="item in menu?.children" :key="`${menu.path}/${item.path}`">
									<template v-if="!item.meta?.exceptCreateMenu">
										<el-menu-item :index="`${menu.path}/${item.path}`" v-if="item.children.length == 0">
		...
		<div class="expand-icon" :class="{ expand: isCollapse }" @click="isCollapse = !isCollapse">
...
const resolveMenu = (menu) => {
	...
	const newMenu = menu.filter((item) => {
		if (userInfo.value?.is_super) {
			...
			return userInfo.value?.permissions?.includes(item.meta.permission) || item.meta?.title == '首页'
		}
	})
...
const menus = computed(() => {
	// 调整顺序
	const news = menuRoutes.filter((item) => !item?.meta?.exceptCreateMenu).sort((a, b) => a?.meta?.sort - b?.meta?.sort)
	return resolveMenu(news)
})
const activeRoute = computed(() => {
	let path = route?.meta?.activeMenu
	if (path) {
		return path
	}
	if (tabStore.activePage?.includes('?')) {
		path = tabStore.activePage?.split('?')[0]
		return path
	}
	return tabStore.activePage
})
