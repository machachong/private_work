	...
	<div class="main-menu">
		...
		<div class="user-menu">
			...
			<div class="user-message">
				...
				<span @click="toggleTheme" v-else>
					<svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
						<circle cx="12.6032" cy="12.1035" r="11.5" fill="#EDF2FF" fill-opacity="0.15" stroke="#4C6BA8" />
						<mask
							id="mask0_473_2266"
							style="mask-type: luminance"
							maskUnits="userSpaceOnUse"
				...
				<div class="menu-help">
					<el-dropdown trigger="hover" placement="right">
						<BokeSvg title="用户手册" color="#2970ff" class="menu-help-icon" :size="22" name="help_shouce"></BokeSvg>
						<template #dropdown>
							<el-dropdown-item @click="goUserManual()"> 开发后台_使用手册 </el-dropdown-item>
						</template>
					</el-dropdown>
				</div>
			</div>
		...
		<div class="menu-list">
			<el-scrollbar height="100%" ref="scrollbarRef">
				<section class="menu-list-content">
					<div
						:ref="
							(el) => {
								innerAppRefs[item.id] = el
							}
						"
						class="menu-item"
...
import settings, { systemTitle, systemId } from '@/utils/config/settings'
...
const goHome = () => {
	window.location.href = settings.HOME_URL
}
// 用户手册
const goUserManual = () => {
	goMenu({
		url: 'https://boke.feishu.cn/wiki/RgJ5wnUuViMFmakcBSWc6BHpnYd?from=from_copylink',
		target: '_blank'
	})
}
onMounted(async () => {
	...
	// 滚动到当前系统的可视区域
	await nextTick()
	const itemRef = innerAppRefs.value[systemId]
	if (itemRef) {
		scrollbarRef.value.setScrollTop(itemRef.offsetTop)
	}

	// const index = innerApp.value.findIndex((item) => item.name === activeMenu.value.name)
	// if (index > 4) scrollbarRef.value.setScrollTop(scrollbarRef.value?.wrapRef?.clientHeight || 300)
})
</script>
	...
	.menu-item {
		...
		&.active {
			...
			.item-img {
				...
				i,
				.icon-develope {
					color: var(--boke-color-main) !important;
				}
			}
		}
	}
