...
<!-- 高级form组件 -->
<template>
	<div class="boke-form" v-show="columns.length">
		<!-- 表单 -->
		<el-form
			ref="form"
			:model="formData"
			...
			:class="{
				'form-oneline': columns?.length <= 4 && mode == 'search',
				'boke-form-add': vertical
			}"
		>
			<div
				class="boke-form__item-container"
				:class="{ vertical: vertical && mode != 'search' }"
			...
			<div class="boke-form__item-btn" v-if="columns?.length <= 4 && !vertical && !multiple && mode == 'search'">
				<slot name="formFooter"></slot>
			</div>
			<!-- 表单最后按钮 -->
			<div class="boke-form__actions" v-if="(columns?.length > 4 && mode == 'search') || multiple">
				<slot name="formFooter"></slot>
			</div>
		</el-form>
	</div>
...
const { columns, formData } = toRefs(props)
...
const onInputFocus = (e, column) => {
	if (column.formProp?.clickFunc) {
		column.formProp.clickFunc(column)
	}
}
