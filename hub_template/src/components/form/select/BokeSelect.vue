<template>
	<el-select
		...
		"
		:disabled="disabled"
		:popper-class="inputFilter || allowChooseAll ? 'my-select-dropdown-input' : 'my-select-dropdown'"
		:style="{ width: width ? width : '', minWidth: minWidth }"
		v-bind="$attrs"
	>
		<!--  -->
		...
		>
			<template v-if="!isGroup">
				...
				>
					...
					<template v-if="isImg">
						<el-space>
							<img style="max-width: 30px; max-height: 30px; width: auto; height: auto" :src="item[itemImg]" alt="" />
							<span>{{ item[itemLabel] }}</span>
						</el-space>
						<span v-if="isDesc" style="float: right; color: var(--el-text-color-secondary)">{{ item[itemDesc] }}</span>
					</template>
					<template v-else>
			...
			<template v-if="isGroup">
				<el-option-group v-for="group in filterOptions" :key="group[groupKey]" :label="group[groupLabel]">
					<el-option
...
export default defineComponent({
	...
	emits: ['change', 'update:modelValue', 'focus', 'filter'],
	setup(props, ctx) {
		...
		const filterShowOptions = computed(() => {
          ...
          */
				if (valueItem && filterLimitOptions.findIndex((item) => item[props.itemValue] == props.modelValue) == -1) {
		...
		function handleFilter(query) {
			if (props.customFilter) {
				return props.customFilterFun(query)
			}
			if (query !== '') {
		...
		function handleCheckChange(val) {
			const checkedCount = val ? val.length : 0
