<!-- 高级页面组件 -->
<template>
	<boke-card :showHeader="true" class="boke-page" :class="{ 'no-card': noCard }">
		<template #header>
			<template v-if="slots.title">
				<slot name="title"></slot>
			</template>
			<div v-if="title" class="h5 p-b-22">{{ title }}</div>
			<BokeForm
				v-show="searchColumns.length != 0"
				ref="formRef"
				:form-data="formData"
...
const emits = defineEmits(['handleSelectionChange', 'search', 'tableOption', 'handleReset'])
// 列表列的配置数据
let { formData, rules, searchColumns, requestFunc, columns, listData, pagination, filterTableData } = toRefs(props)
