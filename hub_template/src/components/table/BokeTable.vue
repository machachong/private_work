		...
		<div class="boke-table" v-show="tableShow">
			...
			<div class="table-header flex-middle table-header-con">
				...
				<choose-column-menu
					...
					:options="columns.filter((item) => !item.type && item.type != 'selection')"
					:teleported="false"
					...
					<template v-for="column in activeColumns" :key="column.prop">
						...
						>
							...
							<template #default="scope" v-if="slots[column.prop]">
								<!-- 优先使用函数式组件 -->
								<component v-if="column?.component" :is="column?.component" :row="scope.row"></component>
...
const emits = defineEmits(['pageChange', 'sizeChange', 'handleSelectionChange', 'tableOption', 'handleRowClick'])
// 列表列的配置数据
...
const activeColumns = computed(() => {
	return columns.value.filter((col) => {
		return showColumns.value?.includes(col.prop) || col.type == 'selection' || col.type == 'expand'
	})
})
watch(
	() => props.columns,
	() => {
		showColumns.value = columns.value
			.filter((item) => !item.type && item.type != 'selection')
			.map((col) => {
