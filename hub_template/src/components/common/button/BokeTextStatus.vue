...
export default {
	...
	props: {
		mode: {
			type: String,
			default: "point" // poinst text
		},
		status: {
			type: String,
			default: "default" // success, fail, warning, open, default
		}
	},
	setup() {}
}
...
.boke-status-text {
	...
	&.fail {
		&::before {
			background-color: var(--boke-color-fail);
		}
	}
	&.warning {
		&::before {
			background-color: var(--boke-color-warning);
		}
	}
	&.success {
		&::before {
