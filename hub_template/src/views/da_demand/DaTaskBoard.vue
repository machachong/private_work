<template>
	<DashboardBase type="da">
		<template #default>
			<boke-card>
				<div class="overview-title">
					<div> </div>
					<!-- <boke-title mode="list">数分需求总览</boke-title> -->
					<el-space>
						<div class="w-250">
							<boke-select
				...
				<section class="chart-part">
					...
					<div class="chart-item chart-bar">
						<boke-title mode="list" h3 class="chart-title">任务类型统计</boke-title>
						<BokeChart :datas="chartData.list3" :options="pieOptions_two"></BokeChart>
					</div>
...
import { getDsOverviewApi } from '@/views/da_demand/service/task_api'
import { useUserStore } from '@/stores'
import DashboardBase from '@/components/demand/DashboardBase.vue'

const { dwUserList, getDaUser } = useDemand()
...
const pieOptions_two = computed(() => {
	return {
		chart_type_id: '1',
		color: ['#F7C123', '#7666FA', '#76CBEC', '#9A68BD', '#2A9A99', '#FF9EC6', '#859E66']
	}
})
...
const getTaskOverview = async () => {
	try {
		...
		chartData.list3 = [
			['需求类型', '分析报告', 'BI', '标签', 'AB实验', '其他'],
			[
				'需求数量',
				items?.type_count?.type1_num,
				items?.type_count?.type2_num,
				items?.type_count?.type3_num,
				items?.type_count?.type4_num,
				items?.type_count?.type5_num
			]
		]
...
onMounted(() => {
	getDaUser().then(() => {
		...
		form.feishu_user_id = keys.includes(userStore.userInfo?.union_id) ? userStore.userInfo?.union_id : ''
		init()
	})
})
</script>
...
.status_acount {
	...
	.top-num {
		...
		.num-item {
			...
			box-shadow: 3px 3px 10px 0px #f2f3f5;
