<template>
	<section>
		...
		<!-- 需要两端更新时提示 -->
		<template
			v-else-if="['需求完成-不符合预期', '需求完成-符合预期', '需求完成-自动验收'].includes(demandStatus) || is_wait"
		>
			<div style="white-space: nowrap">{{ demandStatus }}</div>
		</template>
		<!-- 需要交互的情况 -->
				...
				>
					...
					<template #formSource>
						<el-tree-select
							...
							v-model="entryFormData.menu_source"
							:data="options.menuSource"
							@focus="permissionTreeFocus"
						/>
					</template>
					<template #form_demand_report>
						<BokeWangEditor
							v-model="entryFormData.workflow_feedback"
							height="250px"
			...
			<template #footer>
				<el-row justify="end">
					<el-space>
						...
						<BokeButton type="primary" @click="handleSubmit(formRef)">确定</BokeButton>
					</el-space>
				</el-row>
			</template>
		</BokeDialog>
		<BokeDialog
			...
			title="需求内容不规范"
			v-model="msgDialog.visible"
			width="500px"
			destory-on-close
		>
			<div style="text-align: left; white-space: pre-line; font-size: 14px; line-height: 1.5">
				{{ msgDialog.content }}
			</div>
			<template #footer>
				<el-row justify="end" class="m-t-20">
					<el-space>
						<boke-button @click="msgDialog.visible = false" type="primary">确定</boke-button>
					</el-space>
				</el-row>
			</template>
		</BokeDialog>
...
import { confirm, message, getWangEditorEmpty, showLoading, hideLoading } from '@/utils/tools'
import { demandTypeDict, demandScaleDict } from '../../service/config'
...
const entryFormColumns = shallowRef([])
...
const entryRules = computed(() => {
	...
	return {
		...
		menu_source: [{ required: true, message: '请选择', trigger: 'change' }],
		demand_type: [{ required: true, message: '请选择', trigger: 'change' }],
		demand_scale: [{ required: entryFormData.demand_type == 1, message: '请选择', trigger: 'change' }],
		...
		workflow_feedback: [
			{
				required: passStatus
					.concat(rejectStatus)
					.map((item) => item.value)
					.includes(entryFormData.demand_process_result),
				message: '请输入',
				trigger: 'change'
			}
		]
...
// 动态设置正式验收form
const setCheckForm = () => {
	entryFormColumns.value = defaultFormColumns.filter((item) => {
		const label = entryFormData.demand_process_result == 'process_8_1' ? '验收备注' : '验收报告'
		return item.status?.includes(demandStatus.value) && item.label != label
	})
}
// // 根据对应的节点生成对应的form
...
const defaultFormColumns = [
	{
		...
		formProp: {
			...
			clearable: true,
			onChange: () => {
				entryFormData.menu_source = ''
				options.menuSource = []
				if (entryFormData.system) {
					getMenuSource(entryFormData.system).then((res) => {
	...
	},
	{
		...
		formProp: {
			...
			clearable: true,
			onChange: () => {
				for (let i of defaultFormColumns) {
					if (i?.prop == 'demand_scale') {
						if (entryFormData.demand_type == 1) {
							i.hidden = false
						} else {
							i.hidden = true
						}
...
const demandStatus = computed(() => {
	...
	if (demand_process == 0 || demand_process == 1) {
		...
		(([1, 2].includes(front_status) && [1, 2].includes(back_status)) || demand_source == 2) &&
		demand_process == 5 &&
		task_process == 5 &&
		task_status == 1 &&
		task_user_id == loginUser
	) {
		// 判断只有是前后端 或者 2025-2-28 demand_source == 2 即他是普通需求，直接进入开发状态，因为普通需求部分前后端
		process_status = '完成开发'
	}
	// 分别判断开发合产品的状态
	else if (
	...
	} else if (demand_process == 12) {
		process_status = '需求完成-自动验收'
	} else {
...
//请选择系统平台
const permissionTreeFocus = () => {
	if (!entryFormData.system) {
		message.warning('请选择系统平台')
	}
}
// 处理请求的数据
const updateDemandWorkflow = async () => {
	...
	switch (demandStatus.value) {
		...
		case '更新正式':
			...
			break
		case '正式验收':
			if (!getWangEditorEmpty(workflow_feedback)) {
				const msg = demand_process_result == 'process_8_1' ? '验收报告' : '验收备注'
				return message.error(msg + '不能为空')
			}
			params = {
				task_id,
				demand_process: 8,
...
const sendData = async (value, close = false) => {
	...
	if (demandStatus.value == '需求对接') {
		msgOptions.text = '正在分析需求内容'
	} else {
		msgOptions.text = '正在加载'
	}
	try {
		saveLoading.value = true
		showLoading(msgOptions)
		const res = await api(value, true)
		hideLoading()
		saveLoading.value = false
		if (res?.code === 500) {
			msgDialog.value.visible = true
			msgDialog.value.content = res?.message
			return
		}
		if (!close) {
		...
		// 如果url有mode参数，则删除mode参数，避免在侧边栏参照时，提前赋值拒绝表单内容
		if (route.query.mode) {
...
const initData = () => {
	...
	// 如果url没有mode参数，则清空demand_process_result
	if (!route.query.mode) {
		entryFormData.demand_process_result = ''
	}
}
// 需求认领自动填写默认系统开发人
...
const handleReset = () => {
	...
	entryFormData.workflow_feedback = ''
	dialogVisible.value = false
}
// 所有点击卡片主动打开的拒绝,通过弹窗弹窗均由此方法调用
const showReject = (mode) => {
	const statusArr = mode == 'reject' ? rejectStatus : passStatus
	const timer = setTimeout(() => {
		...
		console.log(entryFormData.demand_process_result)
		clearTimeout(timer)
	}, 500)
}
onMounted(() => {
	optionsStore.getDevOptions()
	if (route.query.mode == 'reject' || route.query.mode == 'approve') {
		showReject(route.query.mode)
	}
})
defineExpose({
	showReject
