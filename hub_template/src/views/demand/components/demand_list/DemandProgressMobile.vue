<template>
	<div class="progress" :class="{ 'no-scroll': type == 'noScroll' }">
				...
				<template v-for="(processData, processIndex) in processDataList" :key="processIndex">
					<div v-for="(item, index) in processData" :key="index">
						<div class="progress-item" :class="{ 'no-first': index != 0 && processData.length > 1 }">
							...
							<div class="item-cell">
								<el-tag v-if="item.task_type" class="item-tag" size="small">
						...
						<!-- <template v-if="getNodeProcess(item)?.length == 0">
							<section
								...
								v-if="
									...
									((index != 0 && processData.length > 1 && index == processData.length - 1) || processData.length == 1)
								"
							>
		...
		<!-- && Object.keys(taskRow).length -->
		<el-affix position="bottom" :offset="22" v-if="![0].includes(demandRow.demand_process)" style="height: 40px">
...
const emits = defineEmits(['ok', 'updateProgress', 'updateProd'])
...
const demandStatusRef = ref()
watch(
	() => [props.taskRow, props.demandRow],
	() => {
		console.log(999, route.query.mode)
		if (route.query.mode == 'reject' || route.query.mode == 'approve') {
			demandStatusRef.value?.showReject(route.query.mode)
		}

		if (props.taskRow?.id && props.demandRow?.id) {
			getCurrentWorkflow({
