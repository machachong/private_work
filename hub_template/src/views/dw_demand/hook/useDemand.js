import { ref } from 'vue'
import { getProjectApi } from '@/api/publicApi'
import {
	getProjectListApi,
	getPoolListApi,
	...
	getTaskProcessApi,
	getTbTaskStatusApi
} from '@/views/dw_demand/service/demand_api'
import { getTaskListApi } from '@/views/dw_demand/service/task_api'
...
export const useDemand = (isDb) => {
	 ...
	 * 判断是来自什么的需求，dw,da,db
	 */
	const getDemandType = computed(() => {
		console.log(route.name)
		if (isDb) {
			return 'db'
		}
		if (route.name?.includes('Db')) {
			return 'db'
		}
		if (route.name?.includes('Data')) {
			return 'da'
		}
		return 'dw'
	})
	...
	const getDemandWorkflowList = async (params) => {
		try {
			const { items = [] } = await getDemandWorkflowListApi(params, false, getDemandType.value)
	...
	const getTaskDetail = async (params) => {
		...
		try {
			...
			const { items = [] } = await getTaskListApi(params, getDemandType.value)
	...
	const getCurrentWorkflow = async (params) => {
		...
		currentTodo.value = ''
		try {
			const { items = [] } = await getCurrentWorkflowApi(params, getDemandType.value)
			let newName = ''
	...
	const getProjectList = async (user_id) => {
		try {
			...
			// console.log(Object.prototype.toString.call(userStore.getUserInfo()))
			projectLoading.value = true
			if (getDemandType.value == 'db') {
				const { items } = await getProjectApi({
					user_id: user_id || userStore.userInfo?.id,
					return_project_name: true,
					need_charge: true // 是否仅返回负责项目
				})
				if (items) {
	...
	// 获取数仓操作人
	const getDwUser = async () => {
		try {
			const type = getDemandType.value == 'dw' ? 1 : 3
			await optionsStore.getDepartmentUser(type)
			dwUserList.value = cloneDeep(
				getDemandType.value == 'dw' ? optionsStore.dwUserOptions : optionsStore.dbUserOptions
			)
		} catch (error) {
			console.log(error)
		}
	...
	const getDemandInfo = async (id) => {
		const res = await getPoolListApi({ id }, getDemandType.value)
