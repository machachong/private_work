						...
						>
							<template v-for="(item, index) in processData" :key="index">
								...
								>
									<div
										class="item-cell"
										v-if="!((index != 0 && processData.length > 1) || item.workflow_process == -4)"
									>
										<el-tag
									...
									<div class="item-cell">
										<template v-if="aduitList.includes(item.workflow_process)">
											{{ getLabel(customUserDict, item.workflow_feishu_user_id) }} 验收
...
const aduitList = [9] // 需要审批的节点
const { getCustomerUser, customUserDict, currentTodo, getCurrentWorkflow, getNodeProcess } = useDemand()
