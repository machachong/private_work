		...
		>
			...
			<template #form_demand_process_result>
				...
				<el-radio-group v-else v-model="formData.demand_process_result" class="no-bg">
					<el-radio v-for="item in newClaimDict" :key="item.value" :label="item.value">{{ item.label }}</el-radio>
				</el-radio-group>
			</template>
			...
			<!-- <template #form_task_content>
				<BokeMdEditor
					v-if="!isMobile"
					ref="mdEditorRefTask"
					:modelValue="formData.task_content"
					:readonly="!canEditDemand1"
				></BokeMdEditor>
        
				<BokeMd v-else :style="{ width: '280px', padding: '0 10px' }" :text="formData.task_content"></BokeMd>
			</template> -->
			<template #form_task_upload>
				<BokeUpload
					...
					uploadUrl="/dw_task/dw_demand_task/attach_file/"
			...
			<template #formUpload>
				<BokeUpload
					...
					uploadUrl="/dw_task/dw_demand_pool/attach_img/"
					tipText="支持粘贴,拖拽上传"
				></BokeUpload>
			</template>
			<template #formTbLink>
...
import {
	...
	dbDemandTypeDict,
	dbClaimDict
} from '@/views/dw_demand/service/config'
import { addPool, updatePool, updateDemandWorkflowApi } from '@/views/dw_demand/service/demand_api'
import { updateTaskApi } from '@/views/dw_demand/service/task_api'
...
import { addDemandTrack, editDemandTrack, claimDemandTrack, finishTaskTrack } from '@/views/dw_demand/service/track'
...
const formData = reactive({
	...
	doc_url: '',
	attach_img: [],
	demand_process_result: getDemandType.value == 'db' ? 'db_demand_claim_1' : 'dw_demand_claim_1', // 认领状态
	...
	demand_description: '',
	milestone_node: '',
	demand_type: undefined // 需求类型dba独有
})
...
const isDb = computed(() => {
	return getDemandType.value == 'db'
})
const isDw = computed(() => {
	return getDemandType.value == 'dw'
})
const newClaimDict = computed(() => {
	if (getDemandType.value == 'db') {
		return dbClaimDict
	} else {
		return claimDict
...
const formColumns = computed(() => {
	return [
		...
		},
		{
			label: '需求类型',
			prop: 'demand_type',
			hidden: !isDb.value || mode.value == 'finish',
			formProp: {
				el: ElSelect,
				required: props.mode == 'claim',
				childEl: ElOption,
				width: '300px',
				source: newDemandType.value?.filter((item) => item.value != 0)
			}
		},
		{
			label: '任务类型',
			prop: 'task_type',
			hidden: canEditDemand6.value || !isDw.value,
			formProp: {
				el: ElSelect,
				required:
					formData.demand_process_result != 'dw_demand_claim_2' &&
					formData.demand_process_result != 'dw_demand_claim_3',
				childEl: ElOption,
				width: '300px',
				source: newDemandType.value?.filter((item) => item.value != 0),
				clearable: true
		...
		},
		{
			...
			hidden:
				mode.value != 'claim' ||
				(formData.demand_process_result != 'dw_demand_claim_3' && getDemandType.value == 'dw') ||
				(formData.demand_process_result != 'db_demand_claim_3' && getDemandType.value == 'db'),
			formProp: {
				el: ElSelect,
				required: true,
				childEl: ElOption,
...
const rules = computed(() => {
	return {
		...
		project_id: [
			{
				required:
					['dw_demand_claim_1', 'db_demand_claim_1'].includes(formData.demand_process_result) &&
					!formData.milestone_node,
				message: '请选择',
				trigger: 'change'
			}
		],
		...
		deadline_time: [
			{
				required: ['dw_demand_claim_1', 'db_demand_claim_1'].includes(formData.demand_process_result),
				message: '请选择',
				trigger: 'change'
			}
		],
		...
		tb_task_status: [
			{
				required:
					isTbClaim.value && ['dw_demand_claim_1', 'db_demand_claim_1'].includes(formData.demand_process_result),
				message: '请选择',
				trigger: 'change'
			}
		]
...
const editStatus = computed(() => {
	...
	// Db需求特殊处理demand_type
	if (
		(!props.demandRow?.demand_type && formData.demand_type) ||
		(!!props.demandRow?.demand_type && props.demandRow?.demand_type != formData.demand_type)
	) {
		return false
	}
	// 特殊处理，附件和执行人要特别进行处理
	...
	for (let i of obj_key) {
		console.log(props?.demandRow[i], formData[i])
...
const changeProject = (val) => {
	const item = projectList.value?.find((item) => item.project_id == val)
	// TB任务切换项目，不清空执行人
...
const updateTask = async (params) => {
	await updateTaskApi(params, getDemandType.value)
}
// 判断当前时间是否超过截止时间
...
const handleSave = async () => {
	...
	await Api({ ...params }, getDemandType.value)
	formData.attach_img = []
	// 2024-5-13 新增成功后置空
	doc_url_name.value = ''
	...
	if (mode.value == 'edit') {
		editDemandTrack(getDemandType.value)
	} else {
		addDemandTrack(getDemandType.value)
	}
}
// 认领
const handleSubmitClaim = async () => {
	try {
		...
		const res = await updateDemandWorkflowApi(params, true, true, getDemandType.value)
		claimDemandTrack(
			{
				需求名称: props.demandRow.demand_name,
				执行人: userStore?.userInfo?.union_id,
				认领状态: getLabel(newClaimDict.value, demand_process_result)
			},
			getDemandType.value
		)
		if (res.data?.status_code == 200) {
			...
			if (props.isMobile) {
				const path = getDemandType.value == 'dw' ? '/mobile/dw_demand_info' : '/mobile/db_demand_info'
				router.push({
					path: path,
					query: {
...
const init = () => {
	...
	try {
		for (let i in formData) {
			if (i == 'attach_img') {
				if (!props?.demandRow?.attach_img) {
					...
					formData.attach_img = props?.demandRow?.attach_img?.split(',').map((item, index) => {
						return {
							name: '粘贴上传' + index,
							url: `${settings.API_URL}/dw_task/dw_demand_pool/attach_img/?attach_img=${item}`,
							response: {
								data: {
									items: item
								}
			...
			} else if (i == 'executor_feishu_user_ids') {
				if (props?.demandRow[i]) {
					formData.executor_feishu_user_ids = props?.demandRow[i]?.split(',')
				} else {
					formData.executor_feishu_user_ids = []
				}
			...
			} else if (i == 'demand_type' && props?.demandRow[i] == 0) {
				formData.demand_type = undefined
			} else {
