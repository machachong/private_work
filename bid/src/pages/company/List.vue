<template>
  <div class="container">
    <el-space>
      <AddItem :bokeDict="bokeDict" @submitOk="getBidList()">新增</AddItem>
...
import { getCompanyListApi, deleteCompanyApi, getTcdeeopyListApi, getIcdeemotListApi } from "./api";
...
const handleDelete = async (item) => {
  try {
    loading.value = true;
    const res = await deleteCompanyApi({
      id: item.id,
    });
    if (res.status === 0) {
